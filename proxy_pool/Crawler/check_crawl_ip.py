import time
from proxy_pool.Crawler.get_proxy import GetProxy
from proxy_pool.Crawler.test_proxy import TestIp
...
class CrawlIp():
    ...
    def crawl(self):
        ...
        while True:
            ...
            other_process = Process(target=self.other_process)
    ...
    def other_process(self):
        for proxy in GetProxy().other_proxy():
            print("代理：%s" % proxy)
            proxy_list = []
            proxy_list.append(proxy)
            TestIp().test_all(proxy_list, "insert")
